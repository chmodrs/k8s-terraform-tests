resource "azurerm_virtual_machine" "k8s-controller" {
    name                  = "k8s-controller"
    location              = "eastus"
    resource_group_name   = "${azurerm_resource_group.myterraformgroup.name}"
    network_interface_ids = ["${azurerm_network_interface.k8s-controller-nic.id}"]
    vm_size               = "Standard_DS1_v2"

    storage_os_disk {
        name              = "k8s-controller-disk"
        caching           = "ReadWrite"
